...
describe('agent', () => {
  ...
  describe('generalization (should extrapolate from patterns)', () => {
    it.concurrent.each([
      ...
      [
        {
          ...
          expected: {
            filters:
              'type:comment AND sentiment.primary:positive AND date.year:2025',
            index: 'commitology_instantsearch_most_positive',
          },
        },
      ],
      [
        {
          question: 'What are the most negative pull requests?',
          expected: {
            filters: 'type:pull',
            index: 'commitology_instantsearch_most_negative',
          },
        },
      ],
      [
