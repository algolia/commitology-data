...
describe('documented examples (must always pass)', () => {
  it.concurrent.each([
    ...
    [
      {
        question:
          'show me everuthong except draft prs, bot comments and chore cmmits',
        expected: {
          filters:
            'NOT pull.state:draft AND NOT user.isBot:true AND NOT commit.state:chore',
        },
      },
    ],
  ])(
...
describe('generalization (should extrapolate from patterns)', () => {
  it.concurrent.each([
    ...
    [
      {
        ...
        expected: {
          filters: 'type:comment AND sentiment.emotions:joy AND date.year:2025',
        },
      },
    ],
    [
    ...
    [
      {
        question: 'Show me the latest open issues where people are frustrated',
        expected: {
          filters: 'issue.state:open AND sentiment.emotions:frustration',
        },
      },
    ],
  ])(
