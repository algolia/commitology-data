...
import { consoleWarn, readJsonUrl, sleep } from 'firost';
...
/**
 * Iterates sequentially through each year from current to oldest item year
 ...
 */
export async function forEachGitHubYearByType(type, callback) {
  const oldestItem = await __.getOldestIssueOrPull(type);
  const oldestYear = dayjs(oldestItem.created_at).year();
...
/**
 * Iterates sequentially through each page of items for a given year
 * @param {string} type - Either 'issue' or 'pr'
 ...
 */
export async function forEachGitHubPageOfYearByType(
  type,
  year,
  callback,
  options = {},
  ...
  const items = await __.getIssuesOrPulls(type, {
    page,
    dateRange,
  });
  ...
  return await forEachGitHubPageOfYearByType(type, year, callback, {
    ...options,
    page: page + 1,
  });
}
...
__ = {
  ...
  async apiCall(path, retryCount = 0) {
    ...
    let headers = null;
    const apiKey = await getKey('GITHUB_TOKEN_COMMITOLOGY_DATA');
    if (apiKey) {
      headers = {
