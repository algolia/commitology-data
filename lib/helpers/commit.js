 ...
 * @param {string} filepath - The filepath to the commit.json file
 * @returns {Promise<object>} A normalized commit object with type, hash (7 chars), title, author, body, date, sentiment, and diff
 */
export async function normalizeCommit(filepath) {
  // Read commit data
  const inputCommit = await readJson(filepath);

  const { author } = inputCommit;
  const body = await normalizeBody(inputCommit.body);
  const date = normalizeDate(inputCommit.date);
  ...
  // Normalize diff
  const diff = normalizeDiff(inputCommit);
