 ...
 */
export async function getSentiment(input, retryCount = 0) {
  const { title } = input;
...
- explanation: Brief explanation (1-2 sentences) of why you chose this sentiment or emotion.
...
IMPORTANT: Output must be valid JSON with all quotes properly escaped.`;
  ...
  try {
    ...
    if (err.code === 'CLAUDE_PARSE_RESPONSE_FAILED' && retryCount < 3) {
      return await getSentiment(input, retryCount + 1);
    }
    throw err;
  }
}
...
__ = {
  ...
  async askClaude(prompt) {
    ...
    // Fix invalid escape sequences (e.g., \*, \., etc.) by escaping the backslash
    // Valid JSON escapes are: \", \\, \/, \b, \f, \n, \r, \t, \uXXXX
    // Replace any backslash not followed by a valid escape character
    ...
    try {
      ...
      console.error('\n=== Claude Parse Error ===');
      console.error('Prompt sent to Claude:');
