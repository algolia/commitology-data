 ...
 * @param {object} options - Options to pass to pMap for controlling concurrency and other behavior
 ...
 */
export async function normalizeIssue(rawIssue) {
  const { number, title, body } = rawIssue;
  const user = normalizeUser(rawIssue.user);
  const date = __.normalizeDate(rawIssue.created_at);
  const labels = __.normalizeLabels(rawIssue.labels);
  const state = __.normalizeState(rawIssue);
