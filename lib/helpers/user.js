...
export const fieldOrder = {
  ...
  output: ['id', 'login', 'name', 'facet', 'isBot', 'isOrganization', 'url'],
};

/**
 * Iterates through all unique user IDs ever mentioned in commit authors, issues, pull requests, and comments, executing a callback for each user ID.
 * @param {Function} callback - Function to execute for each user ID, receives an object with id, index, and max properties
 ...
 */
export async function forEachUserIdEverMentioned(callback, options) {
  // All users from issues and pull requests
  const issueFilepaths = await glob('./**/issue.json', {
    cwd: issueInputDirectory,
  });
  const pullFilepaths = await glob('./**/pull.json', {
    cwd: pullInputDirectory,
  });
  ...
  const commentsFilepaths = await glob('./**/comments.json', {
 ...
 * @param {string} rawUser.name - The user's display name
 * @param {string} rawUser.type - The user's account type
 * @returns {object} The normalized user object with id, login, name, facet, isBot, isOrganization, and url properties
 */
export async function normalizeUser(rawUser) {
  const { id, type } = rawUser;
  const name = rawUser.name || '';
  const login = _.replace(rawUser.login, '[bot]', '');

  const knownUsersAsBots = ['algobot'];

  const isRealBot = type == 'Bot';
  const isUserAsBot = knownUsersAsBots.includes(login);
  const isBot = isRealBot || isUserAsBot;
  const isOrganization = type == 'Organization';

  // Generate URL based on whether it's a real bot or not
  const url = isRealBot
    ? `https://github.com/apps/${login}`
    : `https://github.com/${login}`;

  // Create facet key combining login, name, and id with separator
  // Note: We add spaces around the login and name because we want to consider
  // them as different words, so the highlight can get them. We don't add space
  // for the id because we don't want to find it. We also add a long separator
  // to avoid typos to kick in and find a partial match between login and name
  // for example
  const separator = '▮▮▮▮▮▮▮▮▮▮';
  const facet = `${login} ${separator} ${name} ${separator}${id}`;
