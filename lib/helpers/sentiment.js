...
/**
 * Validates if a raw sentiment response from Claude has valid primary and emotions
 * @param {object} sentiment - The raw sentiment object from Claude to validate
 * @returns {boolean} True if valid, false otherwise
 */
export function isClaudeSentimentValid(sentiment) {
 ...
 * @returns {boolean} True if valid, false otherwise
 */
export function isOutputSentimentValid(sentiment) {
  // First check raw fields
  if (!isClaudeSentimentValid(sentiment)) {
    return false;
  }
...
export async function normalizeSentiment(sentimentPath, reactions = null) {
  ...
  if (!isOutputSentimentValid(result)) {
    throw new Error(
