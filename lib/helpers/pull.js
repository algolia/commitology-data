...
import { normalizeCommentCount } from './comment.js';
...
export async function forEachInputPull(callback, options) {
  const directories = await glob('./**/*/', { cwd: inputDirectory });
  const max = directories.length;

  return await pMap(
    directories,
    async (dirpath, index) => {
      const basicPath = absolute(inputDirectory, dirpath, 'basic.json');
      const detailedPath = absolute(inputDirectory, dirpath, 'detailed.json');
      ...
      const data = {
        ...detailedData,
        reactions: basicData.reactions,
      };

      return await callback({ filepath: dirpath, data, index, max });
    },
    options,
  );
}
...
export async function normalizePull(inputPull) {
  const { number, title, changed_files, additions, deletions } = inputPull;
  ...
  const pull = {
    ...
    pull: {
      ...
      deletedLinesCount: deletions,
    },
  };

  // Add comment count
  const commentCount = await normalizeCommentCount(pull);
  pull.commentCount = commentCount;
