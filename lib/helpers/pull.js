...
import { normalizeDiff } from './diff.js';
import {
...
export async function forEachGitHubYear(callback) {
  return await forEachGitHubYearByType('pr', callback);
}
...
export async function forEachGitHubPageOfYear(year, callback, options = {}) {
  return await forEachGitHubPageOfYearByType('pr', year, callback, options);
}
...
export async function forEachInputPull(callback, options) {
  const pullPaths = await glob('./**/pull.json', { cwd: inputDirectory });
  const max = pullPaths.length;

  return await pMap(
    pullPaths,
    async (pullPath, index) => {
      const detailedPath = _.replace(pullPath, 'pull.json', 'detailed.json');

      const pullData = await readJson(pullPath);
      ...
      return await callback({ filepath: pullPath, data, index, max });
    },
    options,
  );
}
 ...
 * @param {string} filepath - The filepath to the pull.json file
 * @returns {Promise<object>} A normalized pull request object with type, number, title, user, body, date, labels, state, reactions, sentiment
 */
export async function normalizePull(filepath) {
  // Read pull data
  const pullPath = filepath;
  const detailedPath = _.replace(filepath, 'pull.json', 'detailed.json');
  const pullData = await readJson(pullPath);
  const detailedData = await readJson(detailedPath);
  const inputPull = { ...pullData, ...detailedData };
  ...
  // Normalize diff
  const diff = normalizeDiff(inputPull);
