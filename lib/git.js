...
/**
 * Retrieves a unique list of authors from git repository history
 * @returns {Promise<Array<{name: string, email: string}>>} Promise that resolves to an array of author objects containing name and email properties
 */
export async function getAuthorList() {
  const result = await repo.run("log --format='%an▮%ae'");
  return _.chain(result)
    .split('\n')
    .uniq()
    .map((line) => {
